...
| `/kupon` | **Kupon kurucu** — 15 maçlık elle giriş → tüm liglerin korpusunda karne → karnenin en yüksek iki sembolü = kupon → adıyla arşiv |
...
GET  /api/benzer             "bu oranda geçmişte ne oldu" (23 bin maç)
...
- **Veri (Faz 3.5, kapandı)** — `xg.py`/`scripts/build_xg.py`: korpusun kendi
  `sut`/`isabet` sayimindan, `hudl/open-data`in (StatsBomb) 2015/16 dort lig
  kesitine (1.517 mac) karsi kalibre edilmis bir beklenen gol vekili denendi.
  Dısarıda bırakmalı ölçümde piyasayı gecmedi (`kalibre_xg` +0,0002, aralik
  sıfırı kapsıyor — `ISTATISTIK_YOL_HARITASI.md` §3.42) ve modul koddan
  kaldırıldı; olcum kaydi belgede duruyor
...
- **Ortak / gövde** — `ortak.py` (normalizasyon, Wilson, Brier, bantlama) ·
  `payloads.py` (uç gövdeleri, tek kaynak) · `health.py` (22 değişmez) ·
...
python -m pytest                    # tamamı (2.087 test)
