...
def hafta_disarida_birak(fabrika: Fabrika,
                         haftalar: Sequence[Girdi]) -> List[Dict[str, Any]]:
    """Her hafta için: diğerlerinde eğit, o haftada ölç.
    ...
    yapılır (modül başlığı, karar 3).
    """
    out: List[Dict[str, Any]] = []
    for i, hafta in enumerate(haftalar):
        egitim = [h for j, h in enumerate(haftalar) if j != i]
        tahminci = fabrika()
...
def degerlendir(fabrika: Fabrika,
                haftalar: Sequence[Girdi]) -> Dict[str, Any]:
    """Bir tahmincinin hafta dışarıda bırakmalı toplam skoru."""
    kayitlar = hafta_disarida_birak(fabrika, haftalar)
...
def karsilastir(fabrikalar: Optional[Sequence[Fabrika]] = None,
                ...
                haftalar: Optional[Sequence[Girdi]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "bootstrap": {"tekrar": BOOTSTRAP_TEKRAR, "tohum": BOOTSTRAP_TOHUM,
                      "guven": GUVEN},
        "yontem": ("hafta disarida birakmali; bootstrap hafta uzerinden ve "
